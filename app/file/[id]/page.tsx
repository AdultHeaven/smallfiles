...
export default function FilePage({ params }: Props) {
  ...
  const handleReportAbuse = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/abuse-reports', {
        ...
        body: JSON.stringify({ fileId: id, reason: abuseReason }),
      });
  ...
  const getFileIcon = () => {
    ...
    if (isPlayableVideo) return <FileVideo size={48} style={{ color: '#8b5cf6' }} />;
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-app)' }}>
        ...
        <div className="preview-container" style={{ display: 'flex', flexDirection: 'column', gap: '32px', width: '100%', maxWidth: '1080px', margin: '0 auto' }}>
          ...
          <div className="split-layout">
            ...
            <div className="preview-pane">
              ...
              {!isImage && !isPlayableVideo && !isAudio && (
              ...
              <div className="card shadow-premium sidebar-card desktop-only-card">
                ...
                <div>
                  ...
                  <p style={{ color: 'var(--text-secondary)', fontSize: '0.75rem', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    <span>{fileSize !== null ? formatSize(fileSize) : 'CDN Delivery'}</span>
                    {createdAt && (
                      <>
                        <span>•</span>
                        <span>{createdAt}</span>
              ...
              <div className="card sidebar-card mobile-only-card">
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', gap: '12px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', minWidth: 0, flex: 1 }}>
                    <h2 className="mobile-file-title" title={id}>
                      {truncateMiddle(id, 24)}
                    </h2>
                    <span className="mobile-file-meta">
                      {fileSize !== null ? formatSize(fileSize) : 'CDN File'} {createdAt ? `• ${createdAt}` : ''}
                    </span>
                  </div>
                  <button
                    onClick={handleDownload}
                    ...
                    disabled={downloading}
                  >
                    {downloading ? <Loader2 className="animate-spin" size={13} /> : <Download size={13} />}
                    <span>Download</span>
                  </button>
                </div>
          ...
          <div className="card conversion-card animate-fade-in">
            <div className="conversion-info">
              ...
              <div className="conversion-text">
                ...
                <p>
                  Upload folders, documents, images, and videos up to 50 GB completely free. Fast direct cloud transit.
                </p>
              </div>
            </div>
      ...
      {isFullScreen && isImage && (
        <div className="fs-overlay" onClick={() => setIsFullScreen(false)}>
          ...
          <img src={fileUrl} alt={id} className="fs-image" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
